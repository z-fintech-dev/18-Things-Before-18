...
st.title("TruePKR: The Freelancer's Receipt")

# 2. Sidebar for Inputs (Keeping it clean)
...
market_rate = st.sidebar.number_input("Google/Market Rate (PKR)", min_value=0.0, value=280.0 if "USD" in currency else 310.0)

# 3. User Preferences
is_pseb = st.checkbox("I am PSEB Registered (0.25% Tax)")
provider = st.selectbox("Which Bank/App are you using?", ["SadaPay/NayaPay", "Standard Chartered", "Meezan/HBL", "Payoneer (to Bank)"])
...
if provider == "SadaPay/NayaPay":
    ...
    spread_pct = 0.030  # 3.0% (Middleman fee included)
else:
    spread_pct = 0.022  # 2.2% for others

# 5. Calculations (The "Behind the Scenes")
...
# 6. The "Beautiful Receipt" Output
st.divider()
